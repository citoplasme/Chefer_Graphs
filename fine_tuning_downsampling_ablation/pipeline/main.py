...
STORAGE_PATH = '../../outputs/Chefer_importance_downsample_ablation/'
...
def train_and_predict(
  ...
  try:
    ...
    os.makedirs(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}', 'train'), exist_ok = True)
    data_sets.pre_construct_all_graphs_for_split(
      training_df,
      #
      data_path = os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}', 'train'),
      #
    ...
    os.makedirs(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}', 'validation'), exist_ok = True)
    data_sets.pre_construct_all_graphs_for_split(
      validation_df,
      #
      data_path = os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}', 'validation'),
      #
      label_indices = np.unique(training_df['label'].values).tolist(),
      #
      chunk_size = 512,
    ...
    # Clear memory
    shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}', 'train'))
    shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}', 'validation'))
    ...
    if not evaluate_test:
      ...
      del model
      # Delete cache folder
      shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}'))
      ...
      os.makedirs(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}', 'test'), exist_ok = True)
      data_sets.pre_construct_all_graphs_for_split(
        testing_df,
        #
        data_path = os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}', 'test'),
        #
        label_indices = np.unique(training_df['label'].values).tolist(),
        #
        chunk_size = 512,
      ...
      torch.save({
        'state_dict': model.state_dict()
      }, os.path.join(CACHE_PATH, DATASET, f'{trial_number}', f'{SUBSAMPLE_PERCENTAGE}', f'{random_state}.pth.tar'))
      ...
      del model
      # Clear memory
      shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}', 'test'))
      # Delete cache folder
      shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}'))
      del testing_batches
      gc.collect()
      ...
      pd.DataFrame({
        ...
        os.path.join(STORAGE_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{trial_number}', 'metrics.csv'),
        mode = 'a',
        header = not os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{trial_number}', 'metrics.csv')),
        index = False
      )

      os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{trial_number}', f'{random_state}'), exist_ok = True)
      ...
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{trial_number}', f'{random_state}', 'predictions.csv'), index = False)
      ...
      pd.DataFrame(
        ...
        identifier = test_identifiers + best_validation_identifiers,
        split = ['test'] * len(test_labels) + ['validation'] * len(best_validation_labels)
      ).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{trial_number}', f'{random_state}', 'probabilities.csv'), index = False)

      pd.DataFrame({
        'epoch_runtime' : epoch_runtimes,
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{trial_number}', f'{random_state}', 'runtimes.csv'), index = False)
    ...
    print(e, flush = True)
    # Delete cache folder
    if os.path.isdir(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}')):
      shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{edge_threshold}-{node_threshold}'))
...
if __name__ == '__main__':
  ...
  parser.add_argument('--chefer_importance_trial_number', required = True, type = int, help = 'The number of the Chefer importance model to be used as basis for training.')
  ...
  CHEFER_IMPORTANCE_TRIAL_NUMBER = args.chefer_importance_trial_number
  ...
  if not os.path.exists(os.path.join('..', 'fine_tuning', 'models', DATASET, f'{FINE_TUNING_TRIAL_NUMBER}')):
  ...
  top_N_trials = top_N_trials[top_N_trials['number'] == CHEFER_IMPORTANCE_TRIAL_NUMBER]
  ...
  for trial in top_N_trials.itertuples(index = False):
    ...
    for SUBSAMPLE_PERCENTAGE in [0.75, 0.5, 0.25]:
      ...
      PLM = transformers.AutoModelForSequenceClassification.from_pretrained(
        os.path.join('..', 'fine_tuning', 'models', DATASET, f'{FINE_TUNING_TRIAL_NUMBER}', f'{SUBSAMPLE_PERCENTAGE}', f'{SEED}'),
      ...
      os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{trial.number}'), exist_ok = True)
      ...
      for random_state in range(SEED - TEST_RUNS_AROUND_BASE_SEED[0], SEED + TEST_RUNS_AROUND_BASE_SEED[1] + 1):
        ...
        if os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{trial.number}', f'{random_state}', 'predictions.csv')) \
          and os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{trial.number}', f'{random_state}', 'probabilities.csv')):

          predictions = pd.read_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Chefer_importance-{SUBSAMPLE_PERCENTAGE}', f'{trial.number}', f'{random_state}', 'predictions.csv'))
