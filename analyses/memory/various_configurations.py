...
STORAGE_PATH = './statistics/various_configurations/'
os.makedirs(STORAGE_PATH, exist_ok = True)

tokenizer = transformers.AutoTokenizer.from_pretrained('google-bert/bert-base-uncased')

df = pd.concat([
  pd.read_csv('../../data/with_validation_splits/IMDb-1k/train.csv').sample(SAMPLES_PER_DATASET, random_state = SEED).assign(dataset = 'IMDb-1k', split = 'train').reset_index(),
  pd.read_csv('../../data/with_validation_splits/IMDb-1k/validation.csv').sample(SAMPLES_PER_DATASET, random_state = SEED).assign(dataset = 'IMDb-1k', split = 'validation').reset_index(),
  pd.read_csv('../../data/with_validation_splits/IMDb-1k/test.csv').sample(SAMPLES_PER_DATASET, random_state = SEED).assign(dataset = 'IMDb-1k', split = 'test').reset_index(),
  #
...
for window_size in [2, 3, 4, 5]:
  if os.path.isfile(os.path.join(STORAGE_PATH, 'sliding_windows', f'{window_size}.csv')):
    print(f'[UPDATE] Skipping sliding-window graph construction for window size = {window_size}.', flush = True)
    continue
  ...
  pd.DataFrame(sliding_window_statistics, columns = ['dataset', 'split', 'index', 'window_size', 'document_length', 'unique_words', 'token_count', 'node_count', 'node_embedding_dimension', 'edge_count', 'edge_embedding_dimension']) \
    .to_csv(os.path.join(STORAGE_PATH, 'sliding_windows', f'{window_size}.csv'), index = False)
  print(f'[UPDATE] Completed the sliding-window graph construction for window size = {window_size}.', flush = True)
...
for threshold in [0.6, 0.7, 0.8, 0.9, 0.95, 0.99]:
  if os.path.isfile(os.path.join(STORAGE_PATH, 'attention_distillation', f'{threshold}.csv')):
    print(f'[UPDATE] Skipping attention distillation graph construction for threshold = {threshold}.', flush = True)
    continue
  ...
  pd.DataFrame(attention_distillation_statistics, columns = ['dataset', 'split', 'index', 'threshold', 'document_length', 'unique_words', 'token_count', 'node_count', 'node_embedding_dimension', 'edge_count', 'edge_embedding_dimension']) \
    .to_csv(os.path.join(STORAGE_PATH, 'attention_distillation', f'{threshold}.csv'), index = False)
  print(f'[UPDATE] Completed the attention distillation graph construction for threshold = {threshold}.', flush = True)
...
for token_threshold, edge_threshold in list(itertools.product([0.5, 0.6, 0.7, 0.8, 0.9, 0.95], repeat = 2)):
  if os.path.isfile(os.path.join(STORAGE_PATH, 'chefer_importance', f'{token_threshold}-{edge_threshold}.csv')):
    print(f'[UPDATE] Skipping Chefer importance graph construction for token threshold = {token_threshold} and edge threshold = {edge_threshold}.', flush = True)
    continue
  ...
  pd.DataFrame(chefer_importance_statistics, columns = ['dataset', 'split', 'index', 'node_threshold', 'edge_threshold', 'document_length', 'unique_words', 'token_count', 'node_count', 'node_embedding_dimension', 'edge_count', 'edge_embedding_dimension']) \
    .to_csv(os.path.join(STORAGE_PATH, 'chefer_importance', f'{token_threshold}-{edge_threshold}.csv'), index = False)
  print(f'[UPDATE] Completed the Chefer importance graph construction for token threshold = {token_threshold} and edge threshold = {edge_threshold}.', flush = True)
