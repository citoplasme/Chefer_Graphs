...
def remove_isolated_nodes(x, edge_index, device):
  ...
  return edge_index_without_isolated_nodes, x_without_isolated_nodes, new_indices

def construct_PyG_graph_from_raw_attentions(
    text : str,
    label : int | float,
    #split : str,
    index : int,
  ...
  # Drop isolated nodes
  edge_index_unique_PyG_without_isolated_nodes, node_attr_aggregated_without_isolated_nodes, new_indices_mapping = remove_isolated_nodes(
    x = node_attr_aggregated,
    edge_index = edge_index_unique_PyG.t().contiguous(),
    device = device
  )

  # Added for explainability experiments
  mapping = dict()
  for x, y in zip(edge_index_unique[:, 0].cpu().numpy(), edge_index_unique_PyG[:, 0].cpu().numpy()):
    z = new_indices_mapping[y].item()
    if z not in mapping:
      mapping[z] = x
  ...
  if surrogate:
    from_tokens = tokenizer.convert_ids_to_tokens([input_identifiers[0][x] for x in edge_index_unique[:,0]])
    to_tokens = tokenizer.convert_ids_to_tokens([input_identifiers[0][x] for x in edge_index_unique[:,1]])
  else:  
  ...
  # ).to(device) #.to('cpu')
  document_graph_PyG = {
    'x' : node_attr_aggregated_without_isolated_nodes.to('cpu'),
    'y' : torch.tensor([label], dtype = torch.long if classification else torch.float).to('cpu'),
    'edge_index' : edge_index_unique_PyG_without_isolated_nodes.to('cpu'),
    'edge_attr' : edge_attr_aggregated.to('cpu'),
  ...
  # Delete variables
  del new_indices_mapping
  del mapping
  del from_tokens
  del to_tokens
  del edge_index_unique
  del edge_attr_aggregated
  del aggregated_attention
  del cutting_point
  del threshold_mask
  del node_index_unique
  del node_attr_aggregated
  del edge_index_unique_PyG
  del edge_index_unique_PyG_without_isolated_nodes
  del node_attr_aggregated_without_isolated_nodes
