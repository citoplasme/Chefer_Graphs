...
def train_and_predict(
  ...
  try:
    ...
    os.makedirs(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}', f'{0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold)}-{node_threshold}', 'train'), exist_ok = True)
    data_sets.pre_construct_all_graphs_for_split(
      ...
      edge_threshold = 0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold),
      #
      plm = PLM,
      tokenizer = TOKENIZER,
    ...
    os.makedirs(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}', f'{0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold)}-{node_threshold}', 'validation'), exist_ok = True)
    data_sets.pre_construct_all_graphs_for_split(
      validation_df,
      #
    ...
    # Clear memory
    shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}', f'{0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold)}-{node_threshold}', 'train'))
    shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}', f'{0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold)}-{node_threshold}', 'validation'))
    ...
    if not evaluate_test:
      ...
      del model
      # Delete cache folder
      shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}', f'{0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold)}-{node_threshold}'))
      ...
      os.makedirs(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}', f'{0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold)}-{node_threshold}', 'test'), exist_ok = True)
      data_sets.pre_construct_all_graphs_for_split(
        testing_df,
        #
      ...
      del model
      # Clear memory
      shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}', f'{0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold)}-{node_threshold}', 'test'))
      # Delete cache folder
      shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}', f'{0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold)}-{node_threshold}'))
      del testing_batches
      gc.collect()
    ...
    print(e, flush = True)
    # Delete cache folder
    if os.path.isdir(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}', f'{0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold)}-{node_threshold}')):
      shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}', f'{0.0 if ABLATION == 0 else (1.0 if ABLATION == 1 else edge_threshold)}-{node_threshold}'))
...
if __name__ == '__main__':
  ...
  parser.add_argument('--ablation', required = True, type = int, help = 'The ablation to apply during graph construction. Choose an index from the following possibilities: [fully connected first-level, no first-level edges, unit weight edges, no second-level edges, no second- and third-level edges, bi-directional edges to second- and third-level nodes].')
  ...
  if (ABLATION < 0) or (ABLATION > 5):
    raise ValueError('The ablation parameter must be between 0 and 5.')
  ...
  ABLATIONS = ['fully_connected_first_level_edges', 'drop_first_level_edges', 'unit_weight_edges', 'drop_second_level_nodes', 'drop_second_and_third_level_nodes', 'bi_directional_edges_to_second_and_third_level_nodes']
  ...
  UNBOUND_STUDY_NAME = f'{DATASET}-Chefer_importance-{ABLATIONS[ABLATION]}-unbound'
