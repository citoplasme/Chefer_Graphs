...
def train_and_predict(
  ...
  try:
    ...
    data_sets.pre_construct_all_graphs_for_split(
      training_df,
      #
      data_path = os.path.join(SCRATCH_PATH, f'{DATASET}-Sliding_windows', f'{window_size}', 'train'),
      #
      classification = True,
      #
      chunk_size = 512,
    ...
    data_sets.pre_construct_all_graphs_for_split(
      validation_df,
      #
      data_path = os.path.join(SCRATCH_PATH, f'{DATASET}-Sliding_windows', f'{window_size}', 'validation'),
      #
      classification = True,
      #
      chunk_size = 512,
    ...
    if not evaluate_test:
      ...
      data_sets.pre_construct_all_graphs_for_split(
        testing_df,
        #
        data_path = os.path.join(SCRATCH_PATH, f'{DATASET}-Sliding_windows', f'{window_size}', 'test'),
        #
        classification = True,
        #
        chunk_size = 512,
      ...
      del model
      # Clear memory
      shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Sliding_windows', f'{window_size}', 'test'))
      del testing_batches
      gc.collect()
      torch.cuda.empty_cache()
      ...
      pd.DataFrame({
        ...
        os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial_number}', 'metrics.csv'),
        mode = 'a',
        header = not os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial_number}', 'metrics.csv')),
        index = False
      )

      os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial_number}', f'{random_state}'), exist_ok = True)
      ...
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial_number}', f'{random_state}', 'predictions.csv'), index = False)
      ...
      pd.DataFrame(
        ...
        identifier = test_participants + best_validation_participants,
        split = ['test'] * len(test_labels) + ['validation'] * len(best_validation_labels)
      ).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial_number}', f'{random_state}', 'probabilities.csv'), index = False)
...
def objective_function(trial):
  ...
  #
  window_size : int = HYPER_PARAMETERS['window_size']['value'] if HYPER_PARAMETERS['window_size']['fixed'] else (trial.suggest_categorical('window_size', HYPER_PARAMETERS['window_size']['original_search_space']))
  #
...
if __name__ == '__main__':
  ...
  for trial in top_N_trials.itertuples(index = False):
    ...
    os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial.number}'), exist_ok = True)
    ...
    for random_state in range(SEED - TEST_RUNS_AROUND_BASE_SEED[0], SEED + TEST_RUNS_AROUND_BASE_SEED[1] + 1):
      ...
      if os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial.number}', f'{random_state}', 'predictions.csv')) \
        and os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial.number}', f'{random_state}', 'probabilities.csv')) \
        and os.path.isfile(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial.number}', f'{random_state}', 'runtimes.csv')) \
        and os.path.isfile(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial.number}', 'metrics.csv')):

        metrics = pd.read_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial.number}', 'metrics.csv'))[['f1_score_threshold', 'random_state']]
        best_validation_f1_score_threshold = metrics.loc[metrics['random_state'] == random_state, 'f1_score_threshold'].iloc[0]
        
        predictions = pd.read_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial.number}', f'{random_state}', 'predictions.csv'))
        probabilities = pd.read_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Sliding_windows', f'{trial.number}', f'{random_state}', 'probabilities.csv'))
        ...
        test_performance, validation_performance, _, _ = train_and_predict(
          ...
          #
          window_size = HYPER_PARAMETERS['window_size']['value'] if HYPER_PARAMETERS['window_size']['fixed'] else (trial.params_window_size),
          #
