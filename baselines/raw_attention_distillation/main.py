...
def train_and_predict(
  ...
  try:
    ...
    data_sets.pre_construct_all_graphs_for_split(
      training_df,
      #
      data_path = os.path.join(SCRATCH_PATH, f'{DATASET}-Attention_distillation', f'{threshold}', 'train'),
      #
      classification = True,
      #
      chunk_size = 512,
    ...
    data_sets.pre_construct_all_graphs_for_split(
      validation_df,
      #
      data_path = os.path.join(SCRATCH_PATH, f'{DATASET}-Attention_distillation', f'{threshold}', 'validation'),
      #
      classification = True,
      #
      chunk_size = 512,
    ...
    # Clear memory
    shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Attention_distillation', f'{threshold}', 'train'))
    shutil.rmtree(os.path.join(SCRATCH_PATH, f'{DATASET}-Attention_distillation', f'{threshold}', 'validation'))
    ...
    if not evaluate_test:
      ...
      data_sets.pre_construct_all_graphs_for_split(
        testing_df,
        #
        data_path = os.path.join(SCRATCH_PATH, f'{DATASET}-Attention_distillation', f'{threshold}', 'test'),
        #
        classification = True,
        #
        chunk_size = 512,
      ...
      pd.DataFrame({
        ...
        mode = 'a',
        header = not os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Attention_distillation', f'{trial_number}', 'metrics.csv')),
        index = False
      )

      os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-Attention_distillation', f'{trial_number}', f'{random_state}'), exist_ok = True)
...
def objective_function(trial):
  ...
  #
  threshold : float = HYPER_PARAMETERS['threshold']['value'] if HYPER_PARAMETERS['threshold']['fixed'] else (trial.suggest_categorical('threshold', HYPER_PARAMETERS['threshold']['original_search_space']))
  #
...
def unbound_objective_function(trial):
  ...
  #
  threshold : float = HYPER_PARAMETERS['threshold']['value'] if HYPER_PARAMETERS['threshold']['fixed'] else (trial.suggest_float('threshold', *UNBOUND_LIMITS['threshold'], log = HYPER_PARAMETERS['threshold']['log'], step = 0.01))
  #
...
if __name__ == '__main__':
  ...
  for trial in top_N_trials.itertuples(index = False):
    ...
    os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-Attention_distillation', f'{trial.number}'), exist_ok = True)
    ...
    for random_state in range(SEED - TEST_RUNS_AROUND_BASE_SEED[0], SEED + TEST_RUNS_AROUND_BASE_SEED[1] + 1):
      ...
      if os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Attention_distillation', f'{trial.number}', f'{random_state}', 'predictions.csv')) \
        ...
        test_performance, validation_performance, _, _ = train_and_predict(
          ...
          #
          threshold = HYPER_PARAMETERS['threshold']['value'] if HYPER_PARAMETERS['threshold']['fixed'] else (trial.params_threshold),
          #
