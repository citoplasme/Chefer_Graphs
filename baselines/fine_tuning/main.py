...
STORAGE_PATH = '../../outputs/fine_tuning_truncated/'
...
def train_and_predict(
  ...
  try:
    ...
    best_validation_probabilities, best_validation_rows, epoch_runtimes = model_training(
    ...
    if not evaluate_test:
      ...
      pd.DataFrame({
        ...
        os.path.join(STORAGE_PATH, f'{DATASET}-Fine-tuning-Truncated', f'{trial_number}', 'metrics.csv'),
        mode = 'a',
        header = not os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Fine-tuning-Truncated', f'{trial_number}', 'metrics.csv')),
        index = False
      )
      
      os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-Fine-tuning-Truncated', f'{trial_number}', f'{random_state}'), exist_ok = True)
      ...
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Fine-tuning-Truncated', f'{trial_number}', f'{random_state}', 'predictions.csv'), index = False)
      ...
      pd.DataFrame(
        ...
        index = test_rows + best_validation_rows,
        split = ['test'] * len(test_labels) + ['validation'] * len(best_validation_labels)
      ).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Fine-tuning-Truncated', f'{trial_number}', f'{random_state}', 'probabilities.csv'), index = False)

      pd.DataFrame({
        'epoch_runtime' : epoch_runtimes,
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Fine-tuning-Truncated', f'{trial_number}', f'{random_state}', 'runtimes.csv'), index = False)
...
if __name__ == '__main__':
  ...
  STUDY_NAME = f'{DATASET}-Fine-tuning-Truncated'
  ...
  for trial in top_N_trials.itertuples(index = False):
    ...
    os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-Fine-tuning-Truncated', f'{trial.number}'), exist_ok = True)
    ...
    for random_state in range(SEED - TEST_RUNS_AROUND_BASE_SEED[0], SEED + TEST_RUNS_AROUND_BASE_SEED[1] + 1):
      if os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Fine-tuning-Truncated', f'{trial.number}', f'{random_state}', 'predictions.csv')) \
        and os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-Fine-tuning-Truncated', f'{trial.number}', f'{random_state}', 'probabilities.csv')):

        predictions = pd.read_csv(os.path.join(STORAGE_PATH, f'{DATASET}-Fine-tuning-Truncated', f'{trial.number}', f'{random_state}', 'predictions.csv'))
